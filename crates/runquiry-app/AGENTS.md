...
- 流程：`gpui_platform::application().with_assets(Assets)` → `runquiry_ui::locale::extend_component_translations()`（须在 gpui_component::init 之前，进程内一次）→ `gpui_component::init(cx)` → 从可用的绝对配置路径加载设置 → `cx.open_window` 创建窗口，第一级视图必须是 `gpui_component::Root`，内容视图为 `AppShell::new(ShellStartup, …)` → 订阅 ShellEvent 与 `observe_window_bounds` 落盘 → 打开失败走 `eprintln!` + `cx.quit()`（无 unwrap/expect）。
...
无对外 API。窗口行为（B4 起为产品壳层）：初始 1280×800（最小 960×640），`Root` 第一级视图装配 runquiry-ui 的 `AppShell`（侧栏四工作区/工具栏/主数据区/详情区/StatusBar）；启动设置（主题/语言/最后工作区/窗口尺寸）从设置文件恢复，`ShellEvent` 与窗口 bounds 变化写回 allowlist 设置。配置路径只接受平台环境提供的绝对路径；无安全路径时禁用持久化。
...
- `cargo test -p runquiry-app --locked`：14 个测试（settings 13 个：round-trip、损坏回退、allowlist、绝对路径、重复保存、符号链接与 Unix 私有权限等；main 1 个：窗口 bounds 更新持久化尺寸）。
