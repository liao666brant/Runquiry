...
use runquiry_core::{Pid, SocketEntry, validate_socket_entry};
...
pub(crate) fn parse_table(kind: TableKind, buf: &[u8]) -> Result<Vec<SocketRow>, IpTableError> {
...
fn parse_row(kind: TableKind, row: &[u8]) -> Option<SocketRow> {
    let (address, port_raw, remote, state_raw, owner_pid_raw) = match kind {
        TableKind::TcpV4 => (
            ...
            read_u32_le(row, 8)?,
            Some((
                address_v4(row.get(12..16)?.try_into().ok()?),
                read_u32_le(row, 16)?,
            )),
            read_u32_le(row, 0)?,
            read_u32_le(row, 20)?,
        ...
        TableKind::TcpV6 => (
            ...
            read_u32_le(row, 20)?,
            Some((
                address_v6(row.get(24..40)?.try_into().ok()?),
                read_u32_le(row, 44)?,
            )),
            read_u32_le(row, 48)?,
            read_u32_le(row, 52)?,
...
pub(crate) fn to_socket_entry(
    ...
    let entry = SocketEntry {
        ...
        remote_addr: row
            .remote
            .as_ref()
            .and_then(|(addr, raw)| decode_port(*raw).map(|port| format!("{addr}:{port}"))),
