...
pub(super) const ERROR_INVALID_PARAMETER: u32 = 87;
/// `ERROR_INSUFFICIENT_BUFFER`（= 122；表/枚举尺寸探测的预期信号）。
pub(super) const ERROR_INSUFFICIENT_BUFFER: u32 = 122;
...
pub(super) struct Win32Error(pub u32);

impl Win32Error {
    /// 是否为访问拒绝（映射到 [`DiagnosticCode::PermissionDenied`]）。
    #[must_use]
    pub(super) const fn is_access_denied(self) -> bool {
        self.0 == ERROR_ACCESS_DENIED
    }

    /// 是否为「进程已消失或参数非法」类错误（详情采集回退部分结果的信号）。
    #[must_use]
    pub(super) const fn is_target_gone_or_invalid(self) -> bool {
...
#[must_use]
pub(super) fn diagnostic_for(error: Win32Error, subject: &str) -> DiagnosticIssue {
    ...
    DiagnosticIssue::new(
        code,
        format!(
            "{subject} 的 Windows API 调用失败（Win32 错误码 {}）",
            error.0
        ),
    )
}
...
#[must_use]
pub(super) const fn is_ntstatus_failure(status: i32) -> bool {
    status < 0
}
...
mod tests {
    use super::{
        DiagnosticCode, ERROR_ACCESS_DENIED, ERROR_INSUFFICIENT_BUFFER, ERROR_INVALID_PARAMETER,
        ERROR_MORE_DATA, Win32Error, diagnostic_for, is_ntstatus_failure,
    };
    ...
    fn ntstatus_failure_detection() {
        assert!(is_ntstatus_failure(0xC000_0004_u32 as i32));
