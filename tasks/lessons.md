...
- 2026-09-13：`.claude/skills/<name>` 是指向 `.agents/skills/<name>` 的 git 跟踪**相对符号链接**（mode 120000）。npm 包 `skills`（vercel-labs/skills）的 `skills check`（隐藏别名 `update`）在 Windows 上硬编码创建 **junction**（绝对目标），而 Git for Windows 把 junction 当目录遍历——于是 git 报「4 个符号链接被删除 + 约 35 个未跟踪文件」；仓库/系统 gitconfig 的 `core.symlinks=false` 又使符号链接条目被落成普通小文件，进一步掩盖原因。修复：先 `git config core.symlinks true`，用 `cmd /c rmdir "<path>"` 删除 junction（**不要**用 `Remove-Item -Recurse`——它跟随链接，可能连带删掉 `.agents/skills/<name>` 的内容），再 `git checkout-index -f -- .claude/skills/<name>` 让 git 重建真实符号链接。每次运行 `skills` 之后都要重做一遍。
