...
- [ ] **C4 双平台契约回归**
  ...
  - 进度（2026-09-10，Linux/WSL 侧）：在基线 `5a837d0` 上以锁定工具链 1.95.0 串行执行——`cargo fmt --check` 干净、core 109、platform 180（含 `windows_*` 纯解析 77）、ui 69、app 24 全部通过，`cargo test --workspace --all-targets --locked` 385/385 退出码 0，clippy 零 error，`check --examples` 通过（日志与元数据见 `.omo/evidence/c4-linux-verification.md`）。修复 ui 既有失败 `wide_layout_starts_with_a_65_35_split_after_the_sidebar`：`1e44f6e` 侧栏定为 120px 后测试仍按 `px(224.)` 计算，提取共享常量 `SIDEBAR_WIDTH` 统一实现与断言（三处硬编码收敛为一处）；并删除 `processes/tests.rs` 中 clippy `--all-targets` 暴露的未使用 `supported` 变量。**缺口闭合**：batch7b §6.1-2（纯解析套件已跑）、§6.1-3（ui C3 契约与门禁测试已跑）、§6.2 需 Linux 环境项。**C4 仍未完成**：Windows 侧须在 `5a837d0` 之后重跑（其全绿记录早于 macOS 移除提交，两平台须同基线）；差异 #1/#2 待裁决（用户已裁决「接受折中，记录后冻结」，见下）、#3/#4 待实机复核。Linux GUI 交互回归已随 C3 走查完成：四工作区、Partial 横幅、脱敏、双语/主题、窄窗 Sheet、Escape 分层、动作二次确认与取消共 21 张留档截图（`.omo/evidence/linux-gui-verification.md`），并发现确认对话框 2 处 GUI 缺陷（启动时间为 `SystemTime` Debug 格式、缺进程名与用户字段）与 Containers 启动时间列的同源缺陷，**均已修复**：新增 `runquiry-ui::format::format_timestamp`（纯标准库 UTC 格式化，零新增依赖）供两处共用，对话框补齐五要素（`.omo/evidence/fix-confirm-dialog.md`；ui 78/78、工作区 394/394、clippy 零 error、GUI 实测通过，含同轮 code-review 后的重构修订）。**遗留**：全量回归首轮出现一次偶发 `ETXTBSY` flaky（platform `command_runner` 的 `command_argv_elements_pass_through_verbatim_without_shell`，重跑 2 次均通过、单跑 10/10），属既有测试稳定性问题，未修复，是 C4「`--all-targets` 通过」判定的潜在误判源。
  - 进度（2026-09-11，Windows 侧）：在 `ef0a7ea` + ETXTBSY 注释更新树（零行为差异）上以锁定工具链 1.95.0 重跑 `cargo test --workspace --all-targets --locked --no-fail-fast`——35 个测试目标 343 passed / 0 failed（较 Linux 394 少 51 个为 `cfg(unix)` 门控套件不参与 Windows 编译，属预期差集）；`windows_qa` 实机冒烟同步重跑通过（进程 421 / 端口 693 / SCM 证据 1 PID / 受保护进程 `PermissionDenied` 部分结果 / PEB32 环境块 59 项 / 容器 CLI 全未安装）。环境约束：该通道必须 `CARGO_INCREMENTAL=0`（9p 上 rustc 增量锁文件创建失败）。两平台同基线要求已满足。ETXTBSY flaky 完成根因分析并落盘证据（`.omo/evidence/etxtbsy-flaky-analysis.md`）：瞬态愈合实测 ≤ ~5.7ms（高负载），远小于生产 spawn ~75ms 重试窗口，历史单次判定为环境极端尾部，C4 判定口径更新为「再现单点失败先单跑复验再计通过」。证据：`.omo/evidence/c4-windows-verification.md`、`c4-windows-all-targets.log`、`c4-windows-qa-post-baseline.log`。**C4 仍不勾选**：依赖 C2/C3 的 GUI 项（Windows 四工作区交互矩阵、Unsupported 呈现走查）部分已由实机走查留档，剩余项待人工走查（键鼠注入自动化已弃用）。
  - 裁决（2026-09-10，用户）：差异 #1/#2 **接受计划既有的明示折中，记录后冻结**，不扩大范围改造 core/platform 接口；两者按原分类作为已知限制保留，英语界面平台原因维持中文硬编码，resolve/analyze 的环境不可用继续表达为 `InspectError::Unsupported`。
  - 依赖：C2、C3。
