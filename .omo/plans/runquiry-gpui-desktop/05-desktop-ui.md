...
- [ ] **C3 能力矩阵与条件 UI**
  ...
  - 实机走查（2026-09-11，Windows 侧，**部分完成后自动化中止**）：四工作区真实渲染均已走查留档——Processes 真实快照与 Code.exe 详情（祖先链/Sockets/环境变量 57/Partial 横幅含诊断，且**未渲染任何动作按钮**，即「Windows 不渲染可执行动作」实机证据）；Ports 真实快照 + Partial 横幅（258 条 socket 属性主不可知）；Containers `Unavailable` 边界态（info 图标 + 平台原因 + 筛选禁用）；File Locks `Unsupported` 边界态（`—` 图标 + 平台原因「Windows 平台不提供文件锁枚举（parity §10）」原样附显 + 导航入口保留 + 模式/筛选禁用）。英语界面平台诊断中文附显与 C4 差异 #2 冻结折中一致。剩余三项待人工走查：File 目标内联 `InspectError::Unsupported`、名称/PID/端口调查解析流、Ctrl+1..4/Escape 键位与窄窗 Sheet。**中止原因**：WSL→Windows 键鼠注入按焦点投递，竞争时误将输入送入用户其他窗口，经用户指出后停止全部自动化，改为人工操作 + 截图归档。证据：`.omo/evidence/c3-windows-gui-partial/`（README + 5 截图）。**C3 保持未勾选。**
    ...
    - 验证进展（2026-09-10，Linux/WSLg 强制 X11，锁定工具链 1.95.0）：真实 GUI 交互矩阵已在 Linux 侧系统化走查并留档 21 张截图（`.omo/evidence/linux-gui-verification.md` + `linux-gui-verification/`）——四工作区真实快照、Partial 横幅含逐条诊断、空结果态与失败态区分、详情面板全字段、敏感值脱敏（`CLAUDE_CODE_MESSAGING_TOKEN` 已遮蔽）、中英切换即时重绘、浅深主题、960×640 窄窗 Sheet、Escape 分层关闭、进程动作二次确认与取消（取消后目标进程存活、按钮恢复）。gallery 边界态验证 `Unavailable`（`i` 图标、中性色）与 `Unsupported`（`—` 图标）视觉可区分。**本次发现并修复 3 处时间/要素缺陷**：① 确认对话框以 Rust `SystemTime` Debug 格式展示启动时间（用户不可读）；② 同对话框缺少进程名与用户字段（总计划要求「进程名、PID、用户、启动时间、可执行路径」五要素，彼时仅 PID、启动时间、可执行路径）；③ Containers 工作区「启动时间」列同源使用 `format!("{time:?}")`。修复为新增 `crate::format::format_timestamp`（纯标准库 UTC 格式化 `YYYY-MM-DD HH:MM:SS UTC`，占位符 `—`，**零新增依赖**）供对话框与容器表格共用，对话框补 `%{name}`/`%{user}` 占位符——身份三项仍取自冻结的 `ProcessIdentity`，身份冻结语义不变。同轮 code-review 双轴审查（Standards 4 项、Spec 2 项）后修订：提取 `current_action_target()` 消除取数链重复、对话框描述提取为纯函数 `confirmation_description` 并补 2 个测试、`format_optional` 统一占位符（去掉两处 `String::clone()`、`containers_table` 删除本地 `optional()`）、进程名空串同样回退占位符。验证：ui 78/78、`--workspace --all-targets` 394/394、fmt 干净、clippy 零 error、GUI 实测五要素齐备（`.omo/evidence/fix-confirm-dialog.md`）。**C3 仍未勾选**：Windows 侧产品路径的 Unsupported 呈现与能力运行期动态退化撤销在 Linux 无对应场景，需 Windows 实机矩阵或等价覆盖。
