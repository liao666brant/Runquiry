...
- [x] **B1 目标解析与分析管线**
  ...
  - 实施记录（2026-09-04）：
    ...
    - 已知限制：systemd 服务回退的 systemctl 调用属平台/上层（core 仅提供 merge_service_pid 组合语义）；launchd/Windows SCM 判定保留类型与链位置，平台探测属 C2（macOS/C1 已于 2026-09-08 移出 v1 范围，launchd 判定链保留但无平台证据填充）。
