...
- 已知缺口：**Linux/WSL 侧自动验证已于 2026-09-10 闭合**（用户授权，`.omo/evidence/c4-linux-verification.md`）；**Windows 侧须在 `5a837d0` 之后重跑全量**（既有全绿记录早于 macOS 移除提交，两平台须同基线）；C4 差异 #1/#2 待裁决（core `InspectError` 缺 Unavailable 变体、平台原因无稳定原因码）、#3/#4 待实机复核；C3 剩 GUI 交互矩阵的系统化走查留档（标题栏合并与侧栏视觉已用户确认）、C2 剩 GUI 四工作区交互与容器运行时场景；契约 v1 未冻结；B8 已通过（.omo/evidence/batch5-*），不重复验收
- 扫描进度：已完成
