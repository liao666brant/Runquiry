...
- 已知缺口：**Linux/WSL 侧自动验证已于 2026-09-10 闭合**（用户授权，`.omo/evidence/c4-linux-verification.md`）；**Windows 侧须在 `5a837d0` 之后重跑全量**（既有全绿记录早于 macOS 移除提交，两平台须同基线）；C4 差异 #1/#2 待裁决（core `InspectError` 缺 Unavailable 变体、平台原因无稳定原因码）、#3/#4 待实机复核；C3 的 Linux 侧 GUI 交互矩阵已于 2026-09-10 走查留档，剩 Windows 侧 Unsupported 与动态退化场景；C2 剩 GUI 四工作区交互与容器运行时场景；确认对话框与容器启动时间列的 GUI 缺陷已修复（新增 `format` 模块）；**遗留一项偶发 flaky**：全量回归首轮一次 `ETXTBSY`（platform `command_runner` 的 `command_argv_elements_pass_through_verbatim_without_shell`，单跑 10/10、重跑 2 次全绿），系既有测试稳定性问题，是 C4 全量判定的潜在误判源；契约 v1 未冻结；B8 已通过（.omo/evidence/batch5-*），不重复验收
- 扫描进度：已完成
